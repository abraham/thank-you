...
class DeedTest < ActiveSupport::TestCase
  test 'should not save without text' do
    expected_errors = ['User must exist', "User can't be blank", "Name can't be blank", "Text can't be blank"]
    deed = Deed.new
  ...
  test 'should save with three names' do
    deed = Deed.new(text: Faker::Hipster.sentence,
                    name: [Faker::Internet.user_name, Faker::Internet.user_name, Faker::Internet.user_name],
                    user: create(:user))
    assert deed.save
  end
  ...
  test 'should not save with more than three names' do
    ...
    deed = Deed.new(text: Faker::Hipster.sentence,
                    ...
                    user: create(:user))
    assert_not deed.save
    assert_equal ['Name has too many values'], deed.errors.full_messages
  end
end
